...
function getOptionalIfMultitenantConfigFromEnv(
  key: string
): string | undefined {
...
let instance: StorageConfigType;
...
export function getConfig(): StorageConfigType {
  ...
  instance = {
    ...
    headersTimeout: parseInt(
      getOptionalConfigFromEnv("SERVER_HEADERS_TIMEOUT") || "65",
      10
    ),
    ...
    globalS3MaxSockets: parseInt(
      getOptionalConfigFromEnv("GLOBAL_S3_MAX_SOCKETS") || "200",
      10
    ),
    globalS3Protocol: (getOptionalConfigFromEnv("GLOBAL_S3_PROTOCOL") ||
      "https") as "http" | "https",
    ...
    globalS3ForcePathStyle:
      getOptionalConfigFromEnv("GLOBAL_S3_FORCE_PATH_STYLE") === "true",
    ...
    storageBackendType: getConfigFromEnv(
      "STORAGE_BACKEND"
    ...
    //   '',
    urlLengthLimit:
      Number(getOptionalConfigFromEnv("URL_LENGTH_LIMIT")) || 7_500,
    ...
    enableQueueEvents:
      getOptionalConfigFromEnv("ENABLE_QUEUE_EVENTS") === "true",
    ...
    pgQueueRetentionDays: parseInt(
      getOptionalConfigFromEnv("PG_QUEUE_RETENTION_DAYS") || "2",
      10
    ),
    ...
    webhookQueueTeamSize: parseInt(
      getOptionalConfigFromEnv("QUEUE_WEBHOOKS_TEAM_SIZE") || "50"
    ),
    webhookQueueConcurrency: parseInt(
      getOptionalConfigFromEnv("QUEUE_WEBHOOK_CONCURRENCY") || "5"
    ),
    ...
    enableImageTransformation:
      getOptionalConfigFromEnv("ENABLE_IMAGE_TRANSFORMATION") === "true",
    ...
    imgLimits: {
      size: {
        min: parseInt(
          getOptionalConfigFromEnv("IMG_LIMITS_MIN_SIZE") || "1",
          10
        ),
        max: parseInt(
          getOptionalConfigFromEnv("IMG_LIMITS_MAX_SIZE") || "2000",
          10
        ),
      },
    },
    postgrestForwardHeaders: getOptionalConfigFromEnv(
    ...
    enableRateLimiter:
      getOptionalConfigFromEnv("ENABLE_RATE_LIMITER") === "true",
    rateLimiterSkipOnError:
      getOptionalConfigFromEnv("RATE_LIMITER_SKIP_ON_ERROR") === "true",
    rateLimiterDriver:
      getOptionalConfigFromEnv("RATE_LIMITER_DRIVER") || "memory",
    ...
    tusUrlExpiryMs: parseInt(
      getOptionalConfigFromEnv("TUS_URL_EXPIRY_MS") ||
        (1000 * 60 * 60).toString(),
      10
    ),
    ...
    enableDefaultMetrics:
      getOptionalConfigFromEnv("ENABLE_DEFAULT_METRICS") === "true",
  };
  return instance;
}
